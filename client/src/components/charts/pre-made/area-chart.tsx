...
interface PreMadeAreaChartProps {
  data: any[];
  xAxisKey: string;
  yAxisKey: string;
}
...
export function PreMadeAreaChart({ data, xAxisKey, yAxisKey }: PreMadeAreaChartProps) {
  const { toast } = useToast();

  const mutation = useMutation({
    mutationFn: async () => {
      return apiRequest('POST', '/api/ai/generate-chart-insight', { chartType: 'area', chartData: data });
    },
    onSuccess: (data: any) => {
      toast({
        title: "AI Insight",
